...
class ii:
	...
	def setup(self, verbose, csv, dbenable, dburl, dbtoken, dbOStable, dbNETtable):
		...
		# Define log
		try:
			self.log = tabular_log("C:/Program Files/ii/trace.log" if os.name == 'nt' else "~/trace.log", title = "ii" , verbose = self.verbose)
		except:
			self.log = tabular_log("trace.log", title = "ii" ,verbose = self.verbose)
	...
	def get_output(self, array):
		...
		if os.name == 'nt': # If OS == Windows
			...
			return ii.remove_b(subprocess.check_output(cmd, shell=False))
		else:
			return ii.remove_b(subprocess.Popen([self.vboxmanage] + array, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0])
...
def laucher():
	...
	if dburl != None and dbtoken != None and dbOStable != None and dbNETtable != None:
		ii(debug, csv, True, dburl, dbtoken, dbOStable, dbNETtable)
	else:
		ii(debug, csv)
